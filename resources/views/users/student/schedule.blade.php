<x-layouts.app 
  ...
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <p class="font-semibold text-lg text-neutral-800 dark:text-neutral-100">Grade {{ $grade ?? 'N/A' }}</p>
  ...
  {{-- schedule table --}}
  <x-table :headers="['Subject', 'Day', 'Time', 'Room', 'Teacher']">
      @forelse($schedules ?? [] as $schedule)
          <x-table.row>
              ...
              <x-table.cell>{{ $schedule->teacher }}</x-table.cell>
          </x-table.row>
      @empty
          <x-table.row>
