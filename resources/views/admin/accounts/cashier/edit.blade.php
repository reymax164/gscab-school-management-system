<x-layouts.app title="Admin | Edit Cashier" header="Edit Cashier" class="p-4 md:p-6">

    {{-- Top Action Bar --}}
    <div class="flex justify-between items-center mb-4 px-2 md:px-0">
        <a href="{{ route('admin.accounts.cashier.show', $cashier->id) }}"
           class="inline-flex items-center text-sm font-medium text-gray-600 hover:text-blue-900 transition-colors">
            <x-heroicon-o-arrow-left class="w-4 h-4 mr-1" />
            Back to Cashier Details
        </a>
    </div>

    {{-- TODO: routes/admin.php uses an inline closure with no controller; admin.accounts.cashier.update does not exist yet --}}
    <form action="{{ route('admin.accounts.cashier.update', $cashier->id) }}" method="POST">
        @csrf
        @method('PATCH')
            ...
            <div class="px-6 py-5 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 uppercase tracking-tight">
                        {{ $cashier->last_name }}, {{ $cashier->first_name }} {{ $cashier->middle_name ?? '' }}
                    </h2>
                    <p class="text-sm text-gray-500 mt-1">Account created: {{ $cashier->created_at->format('F d, Y h:i A') }}</p>
                </div>
            ...
            {{-- Section 1: Account Details --}}
                <section x-data="{
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-form.input label="Email Address" name="email" type="email" value="{{ old('email', $cashier->email ?? 'N/A') }}" required />
                        <div>
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <x-form.input label="Last Name" name="last_name" value="{{ old('last_name', $cashier->last_name) }}" required />
                        <x-form.input label="First Name" name="first_name" value="{{ old('first_name', $cashier->first_name) }}" required />
                        <x-form.input label="Middle Name" name="middle_name" value="{{ old('middle_name', $cashier->middle_name) }}" />
                        <x-form.input label="Role" name="role_display" value="{{ ucfirst($cashier->role) }}" disabled />
                    </div>
                </section>
