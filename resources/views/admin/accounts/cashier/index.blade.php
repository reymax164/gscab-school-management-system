<x-layouts.app title="Admin | Cashiers" header="Manage Cashiers" class="p-4 md:p-6">

{{-- filter & sort form --}}
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6 px-2 md:px-0">

        <form action="{{ route('admin.accounts.cashier.index') }}" method="GET" class="w-full sm:w-auto flex flex-col sm:flex-row items-start sm:items-end gap-3 flex-wrap">
            ...
            @if(request()->filled('sort_by') || request()->filled('sort_dir'))
                <a href="{{ route('admin.accounts.cashier.index') }}"
        ...
        {{-- add cashier --}}
        <a href="{{ route('admin.accounts.cashier.create') }}"
    ...
    {{-- TODO: routes/admin.php uses an inline closure with no controller; $cashiers (paginated User list where role = cashier) is not yet supplied --}}
    <x-table :headers="['Full Name', 'Email', 'Role', '']">
        @forelse ($cashiers as $cashier)
            <x-table.row>
                <x-table.cell bold>
                    {{ $cashier->last_name }}, {{ $cashier->first_name }}
                    @if($cashier->middle_name)
                        {{ strtoupper(substr($cashier->middle_name, 0, 1)) }}.
                    @endif
                </x-table.cell>
                <x-table.cell>{{ $cashier->email }}</x-table.cell>
                <x-table.cell class="capitalize">{{ $cashier->role }}</x-table.cell>
        ...
        @empty
            <x-table.row>
                <x-table.cell colspan="4" class="text-center text-neutral-500 py-8">No cashier accounts found.</x-table.cell>
            </x-table.row>
        @endforelse
    </x-table>
