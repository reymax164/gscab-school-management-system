<x-layouts.app title="Admin | Edit Teacher" header="Edit Teacher" class="p-4 md:p-6">
            ...
            {{-- Section 1: Account Details --}}
                <section x-data="{
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-form.input label="Email Address" name="email" type="email" value="{{ old('email', $teacher->user->email ?? 'N/A') }}" required />
                        <div>
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <x-form.input label="Employee ID" name="employee_id" value="{{ $teacher->employee_id }}" disabled />
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select name="department_id" id="department_id" required
                                ...
                                @foreach (['Mathematics', 'Science', 'English', 'History', 'Mapeh'] as $department)
                                    <option value="{{ $department }}" {{ old('department_id', $teacher->department_id) == $department ? 'selected' : '' }}>{{ $department }}</option>
                                @endforeach
                            </select>
                        </div>

                        <x-form.input label="Hire Date" name="hire_date" type="date" value="{{ old('hire_date', \Carbon\Carbon::parse($teacher->hire_date)->format('Y-m-d')) }}" required />
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <x-form.input label="Last Name" name="last_name" value="{{ old('last_name', $teacher->user->last_name) }}" required />
                        <x-form.input label="First Name" name="first_name" value="{{ old('first_name', $teacher->user->first_name) }}" required />
                        <x-form.input label="Middle Name" name="middle_name" value="{{ old('middle_name', $teacher->user->middle_name) }}" />
                    </div>
                </section>
