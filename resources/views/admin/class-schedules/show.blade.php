  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
    ...
    <div class="flex flex-wrap gap-x-10 gap-y-3">
      <div class="flex flex-col">
        ...
        <span class="font-semibold text-lg text-blue-900">{{ $classSchedule->grade_level === 'Kinder' ? 'Kindergarten' : 'Grade '.$classSchedule->grade_level }}</span>
      </div>
      ...
      <div class="flex flex-col">
        ...
        <span class="font-semibold text-lg text-blue-900">{{ $classSchedule->classroom->name ?? 'N/A' }}</span>
      </div>

      <div class="flex flex-col">
        <span class="text-xs font-medium text-neutral-500 uppercase tracking-wide">Adviser</span>
        <span class="font-semibold text-lg text-blue-900">{{ $classSchedule->adviser->user->last_name ?? 'N/A' }}{{ isset($classSchedule->adviser->user) ? ', '.$classSchedule->adviser->user->first_name : '' }}</span>
      </div>
    </div>

    <div class="flex items-center gap-2 shrink-0">
      <a href="{{ route('admin.class-schedules.edit', $classSchedule) }}" class="bg-yellow-600 hover:bg-yellow-600/90 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors">Edit</a>
      <a href="{{ route('admin.class-schedules.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium px-4 py-2 rounded-md transition-colors">Back</a>
    </div>

  </div>

  <x-table :headers="['Subject', 'Day', 'Time', 'Room', 'Subject Teacher']">
    @forelse ($classSchedule->subjectSchedules as $subjectSchedule)
      <x-table.row>
